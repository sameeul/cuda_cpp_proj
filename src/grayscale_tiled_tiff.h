...
class NyxusGrayscaleTiffTileLoader : public AbstractTileLoader<DataType> {
 public:
  ...
  void loadTileFromFile(std::shared_ptr<std::vector<DataType>> tile, size_t indexRowGlobalTile,
                        size_t indexColGlobalTile, size_t indexLayerGlobalTile, size_t level) override {
      ...
      case 3:
        switch (bitsPerSample_) {
          ...
          case 64:
            ...
            break;
          default:
            message << "Tile Loader ERROR: The data format is not supported for float, number bits per pixel = "
                    << bitsPerSample_;
            throw(std::runtime_error(message.str()));
        }
        break;
    ...
    if (tileWidth_ > fullWidth_ && tileHeight_ > fullHeight_) {
      ...
      for (size_t r = 0; r < fullHeight_; r++)
        for (size_t c = 0; c < fullWidth_; c++) {
          size_t logOffs = r * tileWidth_ + c, physOffs = r * tileWidth_ + c;
          *(dest + logOffs) = (DataType) * (((FileType*)src) + physOffs);
        }
    } else
    // General case the logical buffer is same size (specifically, tile size) as the physical one even if tileWidth_
    // (e.g. 1024) < fullWidth_ (e.g. 1080)
    {
      size_t n = tileHeight_ * tileWidth_;
      for (size_t i = 0; i < n; i++) *(dest + i) = (DataType) * (((FileType*)src) + i);
    }
  }

  TIFF* tiff_ = nullptr;  ///< Tiff file pointer

  size_t fullHeight_ = 0,  ///< Full height in pixel
